...
export default function SearchPage() {
  ...
  async function handleOrder(item) {
    if (!auth) {
      setOrderState((prev) => ({
        ...prev,
        [item.product_id]: { error: "Faça login pra pedir." },
      }));
      return;
    }
    setOrderState((prev) => ({ ...prev, [item.product_id]: { loading: true } }));
    try {
      const order = await createOrder(
        [{ product_id: item.product_id, product_name: item.name, quantity: 1 }],
        auth.token,
      );
      setOrderState((prev) => ({
        ...prev,
        [item.product_id]: { success: `Pedido #${order.id} feito. ${order.aviso}` },
      }));
    } catch (err) {
      setOrderState((prev) => ({ ...prev, [item.product_id]: { error: err.message } }));
    }
  }
  ...
  return (
    <>
      ...
      <section className="results">
        ...
        {categoryId && (
          <p className="notice">
            Disponibilidade sujeita a confirmação — o preço/estoque é conferido de novo no
            fornecedor no momento do pedido.
          </p>
        )}
        ...
        <ul className="results-grid">
          {pageItems.map((item) => {
            const state = orderState[item.product_id] || {};
            return (
              <li key={item.product_id} className="product-card">
                ...
                <div className="product-card__body">
                  ...
                  {item.availability && (
                    <span className="badge" style={{ "--badge-color": item.availability.color || "#999" }}>
                  ...
                  <button
                    ...
                    disabled={!item.price || state.loading || state.success}
                    onClick={() => handleOrder(item)}
                  >
                    {state.loading ? "Pedindo..." : state.success ? "Pedido feito" : "Pedir"}
                  </button>
                  {state.error === "Faça login pra pedir." ? (
                    <span className="hint hint--error">
                      <Link to="/login">Faça login</Link> pra pedir.
                    </span>
                  ) : (
                    state.error && <span className="hint hint--error">{state.error}</span>
                  )}
                  {state.success && <span className="hint hint--success">{state.success}</span>}
                </div>
              </li>
            );
          })}
